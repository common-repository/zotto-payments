 ...
 */
class WC_Zotto_Bank_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title   = __( 'Bank2Bank', 'zottobankWc1' );
        $this->method_description = __( 'This payment option is applicable only for Bank2Bank Payment in GB.', 'zottobankWc1' );
        ...
        $this->callback_url = WC()->api_request_url( 'callback_url_for_zotto_bank' );
        $this->fail_url = WC()->api_request_url( 'fail_url_for_zotto_bank' );
        // Actions.
       // add_action('woocommerce_api_'.strtolower(get_class($this)), array(&$this, 'callback_handler'));
        add_action( 'woocommerce_api_callback_url_for_zotto_bank', array( $this, 'callback_url_for_zotto_bank' ) );
        add_action( 'woocommerce_api_fail_url_for_zotto_bank', array( $this, 'fail_url_for_zotto_bank' ) );
        add_action('woocommerce_thankyou', array(&$this, 'thankyou_page_for_zotto_bank'));
        if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '>=' ) )
            add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( &$this, 'process_admin_options' ) );
        else
            add_action( 'woocommerce_update_options_payment_gateways', array( &$this, 'process_admin_options' ) );

        add_action( 'woocommerce_receipt_' . $this->id, array(&$this, 'pay_for_orders_zotto_bank'));
    }

    /* Admin Panel Options.*/
    function admin_options() {
        ?>
        <h3><?php _e('Zotto Bank2Bank','zottobank'); ?></h3>
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                'title' => __( 'Enable/Disable', 'zottobankWc1' ),
                'type' => 'checkbox',
                'label' => __( 'Enable zotto Payment', 'zottobankWc1' ),
                'default' => 'no'
            ),
            'title' => array(
                'title' => __( 'Bank2Bank', 'zottobankWc1' ),
                'type' => 'text',
                'description' => __( 'This controls the title which the user sees during checkout.', 'zottobankWc1' ),
                'desc_tip' => true,
                'default' => __( 'Bank2Bank', 'zottobankWc1' )
            ),
            'description' => array(
                'title' => __( 'Description', 'zottobankWc1' ),
                'type' => 'textarea',
            ...
            'instructions' => array(
                'title' => __( 'Instructions', 'zottobankWc1' ),
                'type' => 'textarea',
                'description' => __( 'Instructions that will be added to the thank you page.', 'wcwcCpg2' ),
                'default' => __( 'Instructions', 'zottobankWc1' )
            ),
            'api-key' => array(
                'title' => __( 'API Key', 'zottobankWc1' ),
                'type' => 'text',
                'description' => __( 'Your API key.', 'zottobankWc1' ),
                'default' => __( '', 'zottobankWc1' )
            ),'secret-key' => array(
                'title' => __( 'Secret Key', 'zottobankWc1' ),
                'type' => 'text',
                'description' => __( 'Your Secret key.', 'zottobankWc1' ),
                'default' => __( '', 'zottobankWc1' )
            ),'order-status' => array(
                'title' => __( 'Order Status On Success', 'zottobankWc1' ),
                'type' => 'select',
                ...
                'description' => __( 'Choose Order Status', 'zottobankWc1' ),
                'default' => __( 'Processing', 'zottobankWc1' )
            )
        );
    }
    ...
    public function callback_url_for_zotto_bank() {
    ...
    public function pay_for_orders_zotto_bank( $order_id ) {
        $order = new WC_Order( $order_id );
        $api_key = sanitize_text_field($this->api_key);
        $secret = sanitize_text_field($this->secret_key);
        $redirecttype = 2;
    ...
    function thankyou_page_for_zotto_bank() {
       ...
       /* if(get_post_meta($order_id, '_payment_method', true)=='zottobank') {
    ...
    function fail_url_for_zotto_bank() {
