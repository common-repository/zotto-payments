 ...
 */
class WC_Zotto_Card_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title   = __( 'Zotto Card Payments', 'zottocardWc2' );
        $this->method_description = __( 'For VISA , MasterCard, Maestro , AMEX ,Diners Card Payments', 'zottocardWc2' );
        ...
        $this->callback_url = WC()->api_request_url( 'callback_url_for_zotto_card' );
        $this->fail_url = WC()->api_request_url( 'fail_url_for_zotto_card' );
        // Actions.
       // add_action('woocommerce_api_'.strtolower(get_class($this)), array(&$this, 'callback_handler'));
        add_action( 'woocommerce_api_callback_url_for_zotto_card', array( $this, 'callback_url_for_zotto_card' ) );
        add_action( 'woocommerce_api_fail_url_for_zotto_card', array( $this, 'fail_url_for_zotto_card' ) );
        add_action('woocommerce_thankyou', array(&$this, 'thankyou_page_for_zotto_card'));
        if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '>=' ) )
            add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( &$this, 'process_admin_options' ) );
        else
            add_action( 'woocommerce_update_options_payment_gateways', array( &$this, 'process_admin_options' ) );

        add_action( 'woocommerce_receipt_' . $this->id, array(&$this, 'pay_for_orders_card'));
    }

    /* Admin Panel Options.*/
    function admin_options() {
        ?>
        <h3><?php _e('zotto Card Payment Settings','zottocard'); ?></h3>
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                'title' => __( 'Enable/Disable', 'zottocardWc2' ),
                'type' => 'checkbox',
                'label' => __( 'Enable Zotto Payments', 'zottocardWc2' ),
                'default' => 'no'
            ),
            'title' => array(
                'title' => __( 'Card Payment', 'zottocardWc2' ),
                'type' => 'text',
                'description' => __( 'Zotto Card Payment.', 'zottocardWc2' ),
                'desc_tip' => true,
                'default' => __( 'Card Payment', 'zottocardWc2' )
            ),
            'description' => array(
                'title' => __( 'Description', 'zottocardWc2' ),
                'type' => 'textarea',
                'description' => __( 'This controls the description which the user sees during checkout.', 'zottocardWc2' ),
                'default' => __( 'Pay with zotto pay. We accept all credit card, debit card  payment', 'zottocardWc2' )
            ),
            'instructions' => array(
                'title' => __( 'Instructions', 'zottocardWc2' ),
                'type' => 'textarea',
                'description' => __( 'Instructions that will be added to the thank you page.', 'zottocardWc2' ),
                'default' => __( 'Instructions', 'zottocardWc2' )
            ),
            'api-key' => array(
                'title' => __( 'API Key', 'zottocardWc2' ),
                'type' => 'text',
                'description' => __( 'Your API key.', 'zottocardWc2' ),
                'default' => __( '', 'zottocardWc2' )
            ),'secret-key' => array(
                'title' => __( 'Secret Key', 'zottocardWc2' ),
                'type' => 'text',
                'description' => __( 'Your Secret key.', 'zottocardWc2' ),
                'default' => __( '', 'zottocardWc2' )
            ),'order-status' => array(
                'title' => __( 'Order Status On Success', 'zottocardWc1' ),
                'type' => 'select',
                ...
                'description' => __( 'Choose Order Status', 'zottocardWc1' ),
                'default' => __( 'Processing', 'zottocardWc1' )
            )
        );
    }
    ...
    public function callback_url_for_zotto_card() {
    ...
    public function pay_for_orders_card( $order_id ) {
        $order = new WC_Order( $order_id );
        $api_key = sanitize_text_field($this->api_key);
        $secret = sanitize_text_field($this->secret_key);
        $redirecttype = 1;
    ...
    function thankyou_page_for_zotto_card() {
       ...
       /* if(get_post_meta($order_id, '_payment_method', true)=='zottocard') {
    ...
    function fail_url_for_zotto_card() {
